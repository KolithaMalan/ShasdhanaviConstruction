...
/** Shared Permanent-Employee registry view.
 *  `canManage` gates register/photo actions — HSEQ_OFFICER (Dinesh) & Super
 *  Admin manage; ADMIN_HSEQ (Nuwan) gets a read-only monitoring view. */
export function PermanentEmployeesView({ canManage }: { canManage: boolean }) {
    ...
    <div className="mx-auto max-w-7xl space-y-6">
      <MotionWrapper>
        <PageHeader
          ...
          description="Permanent staff and their no-expiry Site Access Pass."
          actions={canManage ? <AddPermanentEmployeeDialog onCreated={load} /> : undefined}
        />
      </MotionWrapper>
      ...
      <MotionWrapper delay={0.1}>
        {items === null ? <Skeleton className="h-64 w-full" /> :
         items.length === 0 ? (
           <EmptyState icon={BadgeCheck} title="No permanent employees yet" description={canManage ? "Click “Add Permanent Employee” to register staff and issue a pass." : "No permanent employees have been registered yet."} />
         ) : (
