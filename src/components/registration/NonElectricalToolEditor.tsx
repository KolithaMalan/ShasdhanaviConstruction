...
import {
  nonElectricalToolSchema,
  type NonElectricalToolInput,
} from "@/lib/validators";
import { TOOL_UNITS } from "@/types";
import { cn } from "@/lib/utils";

interface Props {
  rows: NonElectricalToolInput[];
  onChange: (rows: NonElectricalToolInput[]) => void;
}

const empty: NonElectricalToolInput = {
  toolName: "",
  category: "",
  quantity: 1,
  unit: "Pieces",
...
export function NonElectricalToolEditor({ rows, onChange }: Props) {
    ...
    <div className="space-y-4">
      ...
      {rows.length === 0 && !adding && (
        <EmptyState icon={Wrench} title="No tools yet"
                    description="Hammers, wrenches, ladders — manual tools and consumables. Optional." />
      )}
        ...
        {rows.map((row, idx) => (
          <motion.div key={idx} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="col-span-6 sm:col-span-3">
              ...
              <div className="text-sm">{row.unit}</div>
            </div>
...
function InlineForm({ onSave, onCancel }: { onSave: (r: NonElectricalToolInput) => void; onCancel: () => void }) {
  const { register, handleSubmit, setValue, watch, formState: { errors } } =
    useForm<NonElectricalToolInput>({ resolver: zodResolver(nonElectricalToolSchema), defaultValues: empty });
  ...
  return (
    <div className="rounded-xl border border-[--color-brand-ocean]/40 bg-card/70 p-5 backdrop-blur-md">
      <div className="mb-4 flex items-center gap-2">
        ...
        <p className="text-sm font-medium">New Tool</p>
      </div>
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <F id="toolName" label="Tool Name" error={errors.toolName?.message}>
          <Input id="toolName" {...register("toolName")} placeholder="Adjustable Spanner" />
        </F>
        <F id="category" label="Category" error={errors.category?.message}>
          <Input id="category" {...register("category")} placeholder="Hand Tools" />
        </F>
        ...
        <F id="unit" label="Unit" error={errors.unit?.message}>
          <Select value={unit}
                  onValueChange={(v) => setValue("unit", v as NonElectricalToolInput["unit"], { shouldValidate: true })}>
            <SelectTrigger id="unit"><SelectValue placeholder="Select unit" /></SelectTrigger>
            <SelectContent>
              {TOOL_UNITS.map((u) => <SelectItem key={u} value={u}>{u}</SelectItem>)}
            </SelectContent>
          </Select>
        </F>
      </div>
