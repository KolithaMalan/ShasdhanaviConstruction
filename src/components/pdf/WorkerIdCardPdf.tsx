...
import type { SerializedWorker } from "@/lib/worker";
  ...
  return (
    <>
      <View style={styles.headerStrip}>
        ...
        <Text style={styles.brandTitle}>Worker Access Pass</Text>
      </View>
...
function CardFront({ worker, logoDataUrl, photoDataUrl }: Omit<Props, "qrDataUrl">) {
  return (
    <View style={styles.card}>
      <Header logoDataUrl={logoDataUrl} />
      <Text style={styles.companyBadge}>{worker.company.toUpperCase()}</Text>
      ...
      <View style={styles.body}>
        <View style={styles.photoFrame}>
          {photoDataUrl ? (
            ...
            <Text style={styles.photoInitials}>{initialsFor(worker.name)}</Text>
          )}
        </View>
        ...
        <View style={styles.rightCol}>
          ...
          <Field label="NIC" value={worker.nicNumber} />
          <Field label="COMPANY" value={worker.company} />
...
function CardBack({ worker, qrDataUrl, logoDataUrl }: Props) {
  return (
    <View style={styles.card}>
      <Header logoDataUrl={logoDataUrl} />
      ...
      <View style={styles.backBody}>
        ...
        <Text style={styles.backId}>{worker.workerId}</Text>
      </View>

      <View style={styles.backFooter}>
        <Text style={styles.backFooterText}>{worker.company} Worker Pass · No Expiry</Text>
...
export function WorkerIdCardPdf({ worker, qrDataUrl, logoDataUrl, photoDataUrl }: Props) {
  return (
    <Document title={`Worker-ID-${worker.workerId}`}>
      <Page size="A6" orientation="landscape" style={styles.page}>
        <CardFront worker={worker} logoDataUrl={logoDataUrl} photoDataUrl={photoDataUrl} />
      </Page>
      <Page size="A6" orientation="landscape" style={styles.page}>
        <CardBack worker={worker} qrDataUrl={qrDataUrl} logoDataUrl={logoDataUrl} />
      </Page>
    </Document>
  );
}
