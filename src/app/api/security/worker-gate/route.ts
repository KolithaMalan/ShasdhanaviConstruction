...
export async function POST(req: Request) {
  ...
  if (data.action === "IN") {
    /* Reuse an OPEN record (lunch re-entry) — don't overwrite morning items.
       Only create a new record if none is open. */
    ...
    visitInfo = { id: String(visit._id), items: visit.items };
  } else {
  ...
  void logAction({
    ...
    description: `${direction} · ${worker.company} worker ${worker.name} (${worker.workerId}) @ ${data.gateLocation}`,
    request: req,
  });
